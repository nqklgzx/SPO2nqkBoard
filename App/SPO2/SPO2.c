...
*                                              内部变量
*********************************************************************************************************/
float SPO2_WaveData[SPO2_ADC_arrMAX] = {0}; //初始化数组
...
static void SPO2_ADC_Read()
{
  ...
  if(SPO2_TM_counter >= SPO2_ADC_TM)                  //达到2ms
  {
    if(ReadADCBuf(&adcData))        //从缓存队列中取出1个数据
    {
      ...
      SPO2_WaveData[WAVE_NUM] = waveData;  //存放到数组
    }
...
//  }
  ...
  if(SPO2_Regular_Flag == SPO2_ADC_NO)
  {
    return;
  }
