...
*                                              宏定义
*********************************************************************************************************/
#define RESP_Reference_Multiple 0.85
#define RESP_Statistic_Num 800
#define RESP_PeakGap 10     //60÷最大呼吸率÷采样时间间隔
/*********************************************************************************************************
*                                              枚举结构体定义
*********************************************************************************************************/
...
{ 
  ...
  for(i = 0;i < RESP_ADC_arrMAX ; i++)
  {
    ...
    if(RESP_WaveData[i] < Minimum)
    {
      Minimum = RESP_WaveData[i];
    }
  }
  RESP_Reference = Minimum +(Maximum-Minimum)* RESP_Reference_Multiple;
...
float RESP_HR_FindMid(int RESP_PeakNum)
{
  ...
  for(j = 0;j < RESP_Peak_TM_DIffer_Count-1;j++)
  {
    ...
    {
      if (RESP_Peak_TM_DIffer[k] > RESP_Peak_TM_DIffer[k+1])//这是升序排法，前一个数和后一个数比较，如果前数大则与后一个数换位置
       {
          temp = RESP_Peak_TM_DIffer[k];
          RESP_Peak_TM_DIffer[k] = RESP_Peak_TM_DIffer[k+1];
          RESP_Peak_TM_DIffer[k+1] = temp;
       }
    }
  }
	//获取中值
if(RESP_Peak_TM_DIffer_Count > 0)
  {
    if(RESP_Peak_TM_DIffer_Count % 2 != 0) // 奇数个点
    {
    ...
    {
        RESP_HR_TM_Mid = (RESP_Peak_TM_DIffer[RESP_Peak_TM_DIffer_Count / 2] + 
                          RESP_Peak_TM_DIffer[RESP_Peak_TM_DIffer_Count / 2 - 1]) / 2.0;
    }
  }
...
void RESP_HR_Cal(float RESP_HR_TM_Mid)
{
  RESP_HeartRate = 60.0 / (RESP_HR_TM_Mid * (0.01 * RESP_ADC_TM));
}
